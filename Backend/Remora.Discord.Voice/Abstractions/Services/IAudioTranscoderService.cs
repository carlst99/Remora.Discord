...
{
    /// <summary>
    /// Represents an interface for transcoding audio segments to/from Opus audio data.
    /// </summary>
    [PublicAPI]
    public interface IAudioTranscoderService
    {
        ...
        /// <summary>
        /// Decodes a sample of Opus audio data.
        /// </summary>
        /// <param name="opusInput">The buffer of Opus audio data to decode.</param>
        /// <param name="sample">The output buffer.</param>
        /// <param name="ct">A <see cref="CancellationToken"/> that can be used to stop the operation.</param>
        /// <returns>A result representing the number of bytes decoded to the output buffer, or an error if the operation failed.</returns>
        Task<Result<int>> DecodeAsync(ReadOnlyMemory<byte> opusInput, Memory<byte> sample, CancellationToken ct = default);
